...
class HostGalaxyRemoval:
    ...
    '''
    def __init__(self, obs_spec, sn_rest_phase, fit_wl_bounds=[4000, 6000], keys=["x", "y", "z"], min_eigenspec=3, max_eigenspec=10, **kwargs):
        ...
        wl_mask = (self.obs_spec[self.keys[0]].value > self.fit_lower_wl) & (self.obs_spec[self.keys[0]].value < self.fit_upper_wl)
    ...
    def fit_spectrum(self):
        ...
        for sn_template in self.sn_templates:
            for num_eigenspec in range(self.min_eigenspec, min(len(self.gal_eigenspec), self.max_eigenspec)  + 1):
                ...
                spec_weighted = self.obs_spec_trimmed[self.keys[1]].value * self.weights
                ...
                gal_model_flux_tot = np.trapz(gal_model, self.obs_spec_trimmed[self.keys[0]])
                ...
                chi2 = np.sum(((self.obs_spec_trimmed[self.keys[1]].value - spec_model) / self.weights)**2)
                ...
                if bic < best_bic:
                    ...
                    self.sn_model = sn_model * self.obs_spec_trimmed[self.keys[1]].unit #TODO "Unit handling issue"
                    self.gal_model = gal_model * self.obs_spec_trimmed[self.keys[1]].unit
    ...
    def remove_galaxy_contamination(self):
        ...
        self.obs_spec_gal_subtracted[self.keys[1]] = self.obs_spec_gal_subtracted[self.keys[1]] - self.gal_model
    ...
    def plot_fit(self, plot_host_free_spec=True, plot_gal_components=True, show=True):
        ...
        if self.spec_model is not None:
            ...
            axes[0].plot(self.obs_spec_trimmed[self.keys[0]], self.spec_model, label="Model")
            axes[0].plot(self.obs_spec_trimmed[self.keys[0]], self.sn_model, label="Model (SN)")
            axes[0].plot(self.obs_spec_trimmed[self.keys[0]], self.gal_model, label="Model (galaxy)")
            axes[0].legend()
            # Residual plot
            axes[1].axhline(0, 0, 1, c="k")
            axes[1].plot(self.obs_spec_trimmed[self.keys[0]], self.obs_spec_trimmed[self.keys[1]].value - self.spec_model, label="Residual (Observed - Model)")
            axes[1].legend()

            if plot_host_free_spec and self.obs_spec_gal_subtracted is not None:
                flux_ratio = np.trapz(self.gal_model, self.obs_spec_trimmed[self.keys[0]]) / np.trapz(self.sn_model, self.obs_spec_trimmed[self.keys[0]])
                ...
                axes[axes_ind].plot(self.obs_spec_gal_subtracted[self.keys[0]], self.gal_model, label=f"Model (galaxy) ({flux_ratio} x SN flux)")
                axes[axes_ind].legend()
                axes_ind += 1

            if plot_gal_components:
                axes[axes_ind].plot(self.obs_spec_trimmed[self.keys[0]], self.gal_model, label="Model (galaxy)")
                if len(self.gal_eigenvals) > 0:
                    for i, eigenspec in enumerate(self.gal_eigenspec):
                        axes[axes_ind].plot(self.obs_spec_trimmed[self.keys[0]],
                                            np.dot(self.gal_eigenvals[i], (eigenspec[self.keys[1]] * self.weights)),  # Must weight eigenspec as eigenvals were found using wieghts
        ...
        for spec in sn_templates:
            for z_diff in [-0.035, -0.03, -0.025, -0.02, -0.015, -0.01, -0.005, -0.0025, -0.001, 0, 0.001, 0.0025, 0.005, 0.01, 0.015, 0.02, 0.025, 0.03, 0.035]:#[-0.035, -0.025, -0.015, -0.005, 0, 0.005, 0.015, 0.025, 0.035, 0.045]:
                if z_diff > 0:
                    ...
                    spec_aligned = align_spec_wave(self.obs_spec_trimmed, spec, keys1=self.keys)  # Note: spec_aligned has same keys as self.keys
                elif z_diff < 0:
                    spec["wave"] *= (1 + abs(z_diff))  # Redshift the spectrum
                    spec_aligned = align_spec_wave(self.obs_spec_trimmed, spec, keys1=self.keys)
                elif z_diff == 0:
                    spec_aligned = align_spec_wave(self.obs_spec_trimmed, spec, keys1=self.keys)
    ...
    def _obtain_gal_eigenspec(self):
        ...
        for spec in gal_eigenspec:
            gal_eigenspec_aligned.append(align_spec_wave(self.obs_spec_trimmed, spec, keys1=self.keys))
    ...
    def _define_sn_template_polynomial(self, wl_fixed=6600):
        ...
        '''
        wl = self.obs_spec_trimmed[self.keys[0]].value
        d_wl = wl - wl_fixed
    ...
    def _get_default_weights(self):
        error_weights = 1 / self.obs_spec_trimmed[self.keys[2]].value
